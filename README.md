...
Use the included `start` script in the project's `package.json`, or the `dev` option to run the application using Nodemon for instant reloads.

- For **npm** users:
```shell
npm run start
npm run dev # uses Nodemon for instant reloads
```

- For **yarn** users:
```shell
yarn run start
yarn run dev # uses Nodemon for instant reloads
```
